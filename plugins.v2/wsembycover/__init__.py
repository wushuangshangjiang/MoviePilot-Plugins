...
class WsEmbyCover(_PluginBase):
    ...
    def __load_style_creator(self, module_name: str, func_name: str):
        ...
        # 兼容历史函数命名，避免运行时因重命名导致崩溃
        fallback_map = {
            ("style_static_1", "create_style_static_1"): "create_style_static_3",
        }
        fallback_name = fallback_map.get((module_name, func_name))
        if fallback_name and hasattr(module, fallback_name):
            logger.warning(f"样式函数 {module_name}.{func_name} 不存在，已回退到 {fallback_name}")
  ...
  华语电影:
    ...
    def __load_title_config(self, yaml_str: str) -> dict:
        try:
            ...
            # 缩进容错：将奇数空格缩进归一化到最近的偶数，兼容 3/5 空格缩进写法
            normalized_lines = []
            indent_fixed = 0
            for raw_line in processed_yaml.split('\n'):
                ...
                if leading % 2 != 0:
                    leading = max(0, leading - 1)
                    indent_fixed += 1
                normalized_lines.append((' ' * leading) + stripped)
            if indent_fixed:
                logger.info(f"标题配置缩进已自动修正 {indent_fixed} 行（建议使用 2 空格缩进）")
